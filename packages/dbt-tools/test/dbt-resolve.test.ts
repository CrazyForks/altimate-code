...
// ---------------------------------------------------------------------------
// Scenario: ALTIMATE_CODE_PYTHON_PATH (injected by vscode-altimate-mcp-server)
// ---------------------------------------------------------------------------
describe("ALTIMATE_CODE_PYTHON_PATH", () => {
  test("resolves dbt as sibling of ALTIMATE_CODE_PYTHON_PATH", () => {
    const venvBin = join(tempDir, "vscode-venv", "bin")
    mkdirSync(venvBin, { recursive: true })
    fakeDbt(venvBin)

    const origVal = process.env.ALTIMATE_CODE_PYTHON_PATH
    process.env.ALTIMATE_CODE_PYTHON_PATH = join(venvBin, "python3")

    try {
      const result = resolveDbt(undefined, undefined)
      expect(result.path).toBe(join(venvBin, "dbt"))
      expect(result.source).toContain("ALTIMATE_CODE_PYTHON_PATH")
    } finally {
      if (origVal !== undefined) process.env.ALTIMATE_CODE_PYTHON_PATH = origVal
      else delete process.env.ALTIMATE_CODE_PYTHON_PATH
    }
  })

  test("ALTIMATE_CODE_PYTHON_PATH loses to ALTIMATE_DBT_PATH", () => {
    const venvBin = join(tempDir, "vscode-venv", "bin")
    mkdirSync(venvBin, { recursive: true })
    fakeDbt(venvBin)

    const customDbt = join(tempDir, "custom-dbt")
    writeFileSync(customDbt, "#!/bin/sh")
    chmodSync(customDbt, 0o755)

    const origPython = process.env.ALTIMATE_CODE_PYTHON_PATH
    const origDbt = process.env.ALTIMATE_DBT_PATH
    process.env.ALTIMATE_CODE_PYTHON_PATH = join(venvBin, "python3")
    ...
    try {
      ...
      if (origDbt !== undefined) process.env.ALTIMATE_DBT_PATH = origDbt
...
// ---------------------------------------------------------------------------
// Scenario: ALTIMATE_CODE_VIRTUAL_ENV (injected by vscode-altimate-mcp-server)
// ---------------------------------------------------------------------------
describe("ALTIMATE_CODE_VIRTUAL_ENV", () => {
  test("resolves dbt from ALTIMATE_CODE_VIRTUAL_ENV/bin/dbt", () => {
    const venvDir = join(tempDir, "vscode-venv")
    const binDir = join(venvDir, "bin")
    mkdirSync(binDir, { recursive: true })
    fakeDbt(binDir)

    const origVal = process.env.ALTIMATE_CODE_VIRTUAL_ENV
    process.env.ALTIMATE_CODE_VIRTUAL_ENV = venvDir

    try {
      const result = resolveDbt(undefined, undefined)
      expect(result.path).toBe(join(binDir, "dbt"))
      expect(result.source).toContain("ALTIMATE_CODE_VIRTUAL_ENV")
    } finally {
      if (origVal !== undefined) process.env.ALTIMATE_CODE_VIRTUAL_ENV = origVal
      else delete process.env.ALTIMATE_CODE_VIRTUAL_ENV
    }
  })

  test("ALTIMATE_CODE_VIRTUAL_ENV loses to ALTIMATE_CODE_PYTHON_PATH sibling", () => {
    const pythonBin = join(tempDir, "python-venv", "bin")
    mkdirSync(pythonBin, { recursive: true })
    const pythonDbt = fakeDbt(pythonBin)
    ...
    process.env.ALTIMATE_CODE_PYTHON_PATH = join(pythonBin, "python3")
    process.env.ALTIMATE_CODE_VIRTUAL_ENV = venvDir

    try {
      const result = resolveDbt(undefined, undefined)
      expect(result.path).toBe(pythonDbt)
      expect(result.source).toContain("ALTIMATE_CODE_PYTHON_PATH")
    } finally {
      if (origPython !== undefined) process.env.ALTIMATE_CODE_PYTHON_PATH = origPython
      else delete process.env.ALTIMATE_CODE_PYTHON_PATH
