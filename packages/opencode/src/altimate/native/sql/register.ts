  ...
  register("sql.fix", async (params) => {
    try {
      ...
      const unfixableMessages = Array.isArray(result.unfixable_errors)
        ...
        : []
      const unfixableError = !result.fixed && unfixableMessages.length > 0 ? unfixableMessages.join("; ") : undefined
  ...
  register("sql.autocomplete", async (params) => {
    try {
      ...
      if (params.table_context?.length) {
        try {
          const ddl = params.table_context.map((t: string) => `CREATE TABLE ${t} (id INT);`).join("\n")
